...
import { GistsGetResponseFiles } from "@octokit/rest";
        ...
        for (const ext of missingExtensions) {
            ...
            try {
                ...
                if (!ext.name.endsWith(".vsix")) {
                    ...
                    notificationCallBack("Skipping extension: '" + ext.name + "'");
                    
                    notificationCallBack("With length: '" + Object.keys(files).join("' '") + "'");
                    var vsixFileName = Object.keys(files).filter(f => f.startsWith(ext.name.substr(0, ext.name.length - 5)))[0];
                    notificationCallBack("With length: " + vsixFileName);

                    let content = files[vsixFileName + ".base64"].content;
                    notificationCallBack("With length: " + content.length);
                }
                notificationCallBack("");
